	...
	switch(mode) {
		case 0 : ///// Pass/allow interrupts to handle stuff
		{
			...
			while(1){
				...
				if(command == 1){
					...
					printf("STARTING DATA READOUT\r\n");
					for(int i = 0; i<N_DATA; i++){
						printf("%f, %d, %d\r\n", linspace(i), POSITIONS[i], CURRENTS[i]);
					}
					printf("END OF DATA READOUT");
					DATA_INDEX  = 0;
					start1 = 0;
					finish1 = 0;
					length1 = 0;
		...
		case 2 : //// TEST POSITION ALGORITHM
		{
			...
			while(1){
				...
				t2 = HAL_GetTick();
				printf("\n\r Position is %d, duration %ld", x,t2-t1);
				HAL_Delay(50);
				ADC_READY = 1;
			}
			break;
		...
		case 4 : /////acceleration test
		{
			...
			while(1){
				...
				if(command){
					...
					for(int i = 0; i<N_DATA; i++){
						printf("\n\r%f, %d, %d",linspace(i), CURRENTS[i], VOLTAGES[i]);
					}
					t1 = HAL_GetTick();
					current = get_current();
					voltage = get_voltage();
					t2 = HAL_GetTick();
					printf("\n\r Current is %d, voltage is %d, I2C time is %ld", current, voltage, t2-t1);
